...
col_1_label_rect = col_1_label.get_rect(center=(COL_1_L_EDGE + CELL_WIDTH/2, 140))
col_2_label = col_head_font.render("Mins", 1, (0, 0, 0))
col_2_label_rect = col_2_label.get_rect(center=(COL_2_L_EDGE + CELL_WIDTH/2, 140))
col_3_label = col_head_font.render("Service", 1, (0, 0, 0))
col_3_label_rect = col_3_label.get_rect(center=(COL_3_L_EDGE + CELL_WIDTH/2, 140))
col_4_label = col_head_font.render("Mins", 1, (0, 0, 0))
col_4_label_rect = col_4_label.get_rect(center=(COL_4_L_EDGE + CELL_WIDTH/2, 140))
...
def update_times: # get new timing data and redraw the screen
	...
	for id, service, mins in services: # iterate through the list
		if id < 6: # which column are we writing to?
			...
			label1_rect = label1.get_rect(center=(COL_1_L_EDGE + CELL_WIDTH/2, Y_POS))
			screen.blit(label1, label1_rect)
			label2 = cell_font.render(mins, 1, (0, 0, 0))
			label2_rect = label2.get_rect(center=(COL_2_L_EDGE + CELL_WIDTH/2, Y_POS))
			screen.blit(label2, label2_rect)
		else:
			Y_POS = Y_POS - (130*5)
			label3 = cell_font.render(service, 1, (0, 0, 0))
			label3_rect = label3.get_rect(center=(COL_3_L_EDGE + CELL_WIDTH/2, Y_POS))
			screen.blit(label3, label3_rect)
			label4 = cell_font.render(mins, 1, (0, 0, 0))
			label4_rect = label4.get_rect(center=(COL_4_L_EDGE + CELL_WIDTH/2, Y_POS))
